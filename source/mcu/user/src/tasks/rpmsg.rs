        ...
        loop {
            ...
            match message.as_ref() {
                ...
                AppMsgRef::DacAddSin50Hz { amp, pha } => {
                    self.control.dac_add_sin_50hz_amp.store(*amp, Ordering::Release);
                    self.control.dac_add_sin_50hz_pha.store(*pha, Ordering::Release);
                }
                AppMsgRef::DacAddSin100Hz { amp, pha } => {
                    self.control.dac_add_sin_100hz_amp.store(*amp, Ordering::Release);
                    self.control.dac_add_sin_100hz_pha.store(*pha, Ordering::Release);
                }
